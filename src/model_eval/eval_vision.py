...
def evaluate_vision_expert():
    ...
    model.load_state_dict(torch.load(MODELS_DIR / "vision_expert_best.pth", map_location=DEVICE))
    model.eval()
    ...
    plot_filename = PLOTS_DIR / "vision_final_evaluation.png"
    plt.savefig(plot_filename)
    
    # Also save the text report for quick reference
    report = classification_report(all_labels, all_preds, target_names=class_names)
    with open(PLOTS_DIR / "vision_classification_report.txt", "w") as f:
        f.write(report)
